...
namespace CustomerManager
{
    ...
    public partial class InvestmentForm : Form
    {
        ...
        Customer customer;
        public InvestmentForm(Customer customer, InvestmentAccounts investmentAccounts)
        {
            InitializeComponent();
        ...
        private void btnWithdraw_Click(object sender, EventArgs e)
        {
            try
            {
                double Amount = double.Parse(txtWithdrawInvestment.Text);
                investmentAccounts.Withdraw(Amount);
                listBox1.Items.Add($"Withdraw: {Amount}");
                txtWithdrawInvestment.Clear();
            }
            catch (FormatException ex)
            {
                MessageBox.Show(ex.Message);
